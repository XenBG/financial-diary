    ...
    >
        <div class="flex items-center justify-start gap-3">
            ...
            <div class="flex w-full items-center justify-start gap-3">
                ...
                <h2 class="text-lg font-medium leading-none text-slate-900 dark:text-zinc-100">Основни разходи</h2>
            </div>
            ...
            <template v-else>
                ...
                >
                    ...
                    <div class="flex items-center justify-start gap-3 p-4">
                        <div class="flex w-full items-center justify-start gap-3">
                            <button class="focus:outline-none" @click="toggleCategoryCollapse('basic', categoryIndex)">
                                <FolderOpenIcon
                    ...
                    <div class="flex w-full flex-col gap-2 border-t border-solid border-slate-300 p-4 dark:border-zinc-600">
                        ...
                        <div class="flex items-start justify-between gap-4 px-1">
                            <div class="flex-start flex items-center gap-2">
                                ...
                                <span class="text-sm font-normal leading-tight text-slate-700 dark:text-zinc-300">
                                    {{ calculateCategorySpentPercentage('basic', categoryIndex) }}% са изразходвани:
                                </span>
                            </div>
                        ...
                        <div class="flex items-start justify-between gap-4 px-1">
                            <div class="flex-start flex items-center gap-2">
                                ...
                                <span class="text-sm font-normal leading-tight text-slate-700 dark:text-zinc-300">
                                    {{ formatCurrency(100 - calculateCategorySpentPercentage('basic', categoryIndex)) }}%
                                    оставащи:
                                </span>
                            </div>
                            <span class="text-right text-sm font-semibold leading-tight text-slate-700 dark:text-zinc-300">
                                {{
                                    formatCurrency(
                                        getCategoryTotal('basic', categoryIndex) - getCategorySpent('basic', categoryIndex),
                                    )
                                }}
                                {{ CURRENCY }}
                            </span>
...
<script setup>
    ...
    // State
    const basicExpenses = computed(() => diaryStore.basicExpenses)
    ...
    const openTransactionsModal = (section, categoryIndex, expenseIndex) => {
        ...
        modalsStore.currentCategoryIndex = categoryIndex
        modalsStore.currentExpenseIndex = expenseIndex
        modalsStore.currentExpense = basicExpenses.value[categoryIndex].expenses[expenseIndex]
